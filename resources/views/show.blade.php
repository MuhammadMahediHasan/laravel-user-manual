...
<div class="user-manual__shell">
    <div
        id="user-manual-sidebar-overlay"
        class="user-manual__overlay"
        data-user-manual-toggle="sidebar"
    ></div>
    ...
    <aside id="user-manual-sidebar" class="user-manual__sidebar">
        ...
        <div class="user-manual__sidebar-footer">
            @auth
                <a href="{{ url(ManualConfig::string('user-manual.ui.back_url', '/dashboard')) }}" class="user-manual__footer-link">
                    ...
                    {{ __('user-manual::messages.back_to_dashboard') }}
                </a>
            @else
                <a href="{{ url(ManualConfig::string('user-manual.ui.login_url', '/login')) }}" class="user-manual__footer-link">
    ...
    <div class="user-manual__main">
        <header class="user-manual__header">
            <div class="user-manual__header-start">
                <button
                    type="button"
                    class="user-manual__menu-button"
                    data-user-manual-toggle="sidebar"
                ...
                <nav class="user-manual__breadcrumb" aria-label="Breadcrumb">
                    <a href="{{ route($routeName, ['locale' => $locale, 'page' => ManualConfig::string('user-manual.default_page', 'introduction')]) }}">{{ __('user-manual::messages.title') }}</a>
            ...
            <div class="user-manual__header-end">
                @if (count($locales) > 1)
                    <div class="user-manual__locale-switcher">
                        @foreach ($locales as $availableLocale)
                            <a
                                href="{{ route($routeName, ['locale' => $availableLocale, 'page' => $page]) }}"
                                @class([
                ...
                <a
                    href="{{ url(ManualConfig::string('user-manual.ui.home_url', '/')) }}"
                    target="_blank"
                    rel="noopener noreferrer"
                    title="{{ __('user-manual::messages.go_to_landing') }}"
